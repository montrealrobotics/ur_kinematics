...
class kinematics_model():
    ...
    def homogeneous_transformation_i(self, theta_i, alpha_i, a_i, d_i):
        ...
        A_i = np.array(
            [np.cos(theta_i), -np.sin(theta_i) * np.cos(alpha_i), np.sin(theta_i) * np.sin(alpha_i),
             a_i * np.cos(theta_i), \
    ...
    def homogeneous_transformation_i_var_alpha(self, i, alpha_i):
        ...
        Returns
            A_i (np array 4*4)-> homogeneous transformation for joint i
        """
        return self.homogeneous_transformation_i(theta_i=self.theta[i], alpha_i=alpha_i, a_i=self.a[i],
                                                 d_i=self.d[i])
    ...
    def homogeneous_transformation_i_var_a(self, i, a_i):
        ...
        Returns
            A_i (np array 4*4)-> homogeneous transformation for joint i
        """
        return self.homogeneous_transformation_i(theta_i=self.theta[i], alpha_i=self.alpha[i], a_i=a_i,
                                                 d_i=self.d[i])
    ...
    def homogeneous_transformation_i_var_d(self, i, d_i):
        ...
        Returns
            A_i (np array 4*4)-> homogeneous transformation for joint i
        """
        return self.homogeneous_transformation_i(theta_i=self.theta[i], alpha_i=self.alpha[i], a_i=self.a[i],
                                                 d_i=d_i)
    ...
    def forward_kin(self, joint_variables):
        ...
        T_matrix = self.get_transformation_matrix(joint_variables=joints_without_offset, start_joint=0,
                                                  end_joint=self.number_of_joints - 1)
    ...
    def inverse_kin(self, pose, orientation):
        ...
        for i in range(len(theta1_possible_values)):
            ...
            T_01 = self.get_transformation_matrix(joint_variables=[theta1_possible_values[i], 0, 0, 0, 0, 0],
            ...
            # theta6
            theta6_possible_values[i * 2] = np.arctan2(-pose_full_form_frame1[2, 1] / np.sin(theta5_possible_values[i]),
                                                       pose_full_form_frame1[2, 0] / np.sin(theta5_possible_values[i]))
            theta6_possible_values[i * 2 + 1] = np.arctan2(
                -pose_full_form_frame1[2, 1] / np.sin(-theta5_possible_values[i]),
                pose_full_form_frame1[2, 0] / np.sin(-theta5_possible_values[i]))
        ...
        for combination_theta1_theta6 in range(len(theta6_possible_values)):
            ...
            # homogeneouos transformation between frames 0 and 1
            T_01 = self.get_transformation_matrix(joint_variables=self.joints_ik[hypothesis_index, :], start_joint=0,
                                                  end_joint=0)
            # homogeneous transformation 1->0 to obtain desired point in the frame 1
            T_10 = np.linalg.inv(T_01)
            # desired pose and orientation in the frame 1
            ...
            T_56 = self.get_transformation_matrix(joint_variables=self.joints_ik[hypothesis_index, :], start_joint=5,
                                                  end_joint=5)
            # homogeneouos transformation between frames 4 and 5
            T_45 = self.get_transformation_matrix(joint_variables=self.joints_ik[hypothesis_index, :], start_joint=4,
                                                  end_joint=4)
            ...
            if coeff > 1 or coeff < -1:
                ...
                theta3 = np.arccos(
                    (np.linalg.norm(P_13) ** 2 - self.a[1] ** 2 - self.a[2] ** 2) / (2 * self.a[1] * self.a[2]))
        ...
        for hypothesis_index in range(self.joints_ik.shape[0]):
            ...
            # theta 2
            self.joints_ik[hypothesis_index, 1] = -np.arctan2(P_13[1], -P_13[0]) + np.arcsin(
                self.a[2] * np.sin(self.joints_ik[hypothesis_index, 2]) / np.linalg.norm(P_13))

            # theta 4
            # homogeneouos transformation between frames 2 and 3
            T_23 = self.get_transformation_matrix(joint_variables=self.joints_ik[hypothesis_index, :], start_joint=2,
                                                  end_joint=2)
            ...
            T_12 = self.get_transformation_matrix(joint_variables=self.joints_ik[hypothesis_index, :], start_joint=1,
                                                  end_joint=1)
